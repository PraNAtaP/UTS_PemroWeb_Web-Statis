...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($content)) {
        ...
        $result = pg_query($dbconn, "INSERT INTO articles (title, content) VALUES ('$title', '$content') RETURNING id");
        
        if ($result) {
            $newArticle = pg_fetch_assoc($result);
            $success = 'Artikel berhasil dibuat!';
            header('refresh:1;url=article_detail.php?id=' . $newArticle['id']);
            exit;
        } else {
            $error = 'Gagal membuat artikel: ' . pg_last_error($dbconn);
        }
    }
}
?>
...
<head>
    ...
    <title>Create Article - Pranata</title>
        ...
        <div class="article fade-in" style="max-width: 800px; margin: 0 auto;">
            ...
            <form method="POST" action="article_create.php">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="artikel.php" class="btn btn-secondary">❌ Cancel</a>
                </div>
            </form>
        </div>
    </div>
